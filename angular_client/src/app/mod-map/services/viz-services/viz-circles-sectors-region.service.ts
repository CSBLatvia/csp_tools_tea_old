...
export class VizCirclesSectorsRegionService  implements IVizService{
  ...
  public loadData(route:RouteVO):void{
    this.route = route;
    const url:string = this.serviceURL+'?db=viz-circles-sectors-region&m1='+route.M1+'&m2='+route.M2+'&m3='+route.M3+'&m4='+route.M4+'&t1='+route.T1+'&t2='+route.T2+'&year='+route.year;
    this.data=[];
    this.ids=[];
    this.colors=[];
  ...
  private parseData(obj:any,time:string):void{
    // this.logger.log('*********************');
    // this.logger.log('VIZ-CIRCLES-SECTORS-REGION- parseData - time:'+time);
    // this.logger.dir(obj);
